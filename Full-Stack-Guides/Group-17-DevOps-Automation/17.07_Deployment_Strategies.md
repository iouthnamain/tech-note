...
3. [Choosing A Strategy / Chọn chiến lược](#choosing-a-strategy--chọn-chiến-lược)
4. [Rollback / Rollback](#rollback--rollback)
...
## Choosing A Strategy / Chọn chiến lược

### Quick Selection Guide / Hướng dẫn chọn nhanh

- blue-green: safest switch when you can afford duplicate environments
- canary: best when you want gradual risk exposure
- rolling: simplest for many orchestrated systems

### Decision Factors / Yếu tố quyết định
...
### Why Rollback Matters / Tại sao rollback quan trọng

Deployment strategy is incomplete without fast recovery from bad releases.

### Example 2: Rollback Flow / Ví dụ 2: Luồng rollback

```mermaid
flowchart TD
    DEPLOY[Deploy New Version] --> MON[Monitor]
    MON -->|healthy| KEEP[Keep Release]
    MON -->|regression| ROLLBACK[Rollback]
    ROLLBACK --> VERIFY[Verify Stability]
```
...
6. **Coordinate database changes** - Deploy strategy and schema changes must align
7. **Use health checks** - Traffic should shift only to healthy instances
8. **Prefer measured rollout** - Release confidence should come from telemetry
...
- **Selection**: Strategy depends on cost, risk, and observability
- **Rollback**: Recovery speed is part of the strategy design
