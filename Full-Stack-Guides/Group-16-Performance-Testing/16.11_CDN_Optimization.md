# 16.11 CDN Optimization / Tối ưu CDN

## Table of Contents / Mục lục
1. [Introduction / Giới thiệu](#introduction--giới-thiệu)
2. [CDN Configuration / Cấu hình CDN](#cdn-configuration--cấu-hình-cdn)
3. [Caching Strategy / Chiến lược cache](#caching-strategy--chiến-lược-cache)
4. [Invalidation / Invalidate cache](#invalidation--invalidate-cache)
5. [Best Practices / Thực hành tốt nhất](#best-practices--thực-hành-tốt-nhất)
6. [Summary / Tóm tắt](#summary--tóm-tắt)

---

## Introduction / Giới thiệu

### Overview / Tổng quan

**English**: CDN improves content delivery speed. Learn to configure CDN, cache static assets, and optimize content delivery.

**Vietnamese**: CDN cải thiện tốc độ phân phối nội dung. Học cách cấu hình CDN, cache static assets và tối ưu phân phối nội dung.

### CDN Flow / Luồng CDN

```mermaid
flowchart TD
    User[User] --> CDN[CDN Edge]
    CDN --> Cache{Cached?}
    Cache -->|Yes| Return[Return Cached]
    Cache -->|No| Origin[Origin Server]
    Origin --> CDN
    CDN --> Return
    
    style User fill:#90EE90
    style Return fill:#e1f5ff
```

---

## CDN Configuration / Cấu hình CDN

### Example 1: CDN Setup / Ví dụ 1: Thiết lập CDN

```typescript
// CDN configuration / Cấu hình CDN
const cdnConfig = {
  staticAssets: {
    cacheControl: 'public, max-age=31536000', // 1 year / 1 năm
    files: ['*.js', '*.css', '*.png', '*.jpg']
  },
  apiResponses: {
    cacheControl: 'public, max-age=300', // 5 minutes / 5 phút
    vary: 'Accept-Encoding'
  },
  html: {
    cacheControl: 'no-cache',
    revalidate: 60 // ISR / ISR
  }
};

// Next.js CDN / CDN Next.js
// next.config.js
module.exports = {
  assetPrefix: 'https://cdn.example.com',
  images: {
    domains: ['cdn.example.com']
  }
};
```

### CDN Delivery Path / Luồng phân phối CDN

```mermaid
flowchart LR
    BROWSER[Browser] --> EDGE[CDN Edge]
    EDGE -->|hit| FAST[Fast Response]
    EDGE -->|miss| ORIGIN[Origin]
    ORIGIN --> EDGE
    EDGE --> FAST
```

---

## Caching Strategy / Chiến lược cache

### What To Cache Aggressively / Những gì nên cache mạnh

- versioned JavaScript bundles
- CSS files
- images
- fonts

### What To Cache Carefully / Những gì nên cache cẩn thận

- HTML pages
- API responses
- personalized content

---

## Invalidation / Invalidate cache

### Why Invalidation Matters / Tại sao invalidate quan trọng

Fast delivery is only useful if users receive correct content.

### Common Approaches / Cách làm phổ biến

- versioned asset filenames
- explicit purge after deploy
- short TTL for changing content
- stale-while-revalidate where supported

---

## Best Practices / Thực hành tốt nhất

1. **Cache static assets** - Long TTL for static files
2. **Cache headers** - Proper cache-control
3. **Compression** - Enable gzip/brotli
4. **Edge locations** - Choose close locations
5. **Purge cache** - Invalidate when needed
6. **Version assets** - Prefer content hashing for static files
7. **Separate static and dynamic policy** - Not all content should share TTL
8. **Measure hit rate** - CDN performance should be observable

---

## Summary / Tóm tắt

### Key Takeaways / Điểm chính

- **Purpose**: Faster content delivery
- **Configuration**: Cache headers
- **Static assets**: Long cache TTL
- **Dynamic content**: Shorter TTL
- **Invalidation**: Freshness rules are part of CDN design
- **Strategy**: Cache policy depends on content type and change frequency

### Next Steps / Bước tiếp theo

- [16.12 Memory Optimization](./16.12_Memory_Optimization.md) - Next: Memory Optimization

---

**Last Updated / Cập nhật lần cuối**: 2024

