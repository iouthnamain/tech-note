...
CLASS zcl_bug_request DEFINITION
  ...
  PRIVATE SECTION.
    ...
    " Logs a system message for monitoring purposes (e.g., workflow errors).
    METHODS log_message
      IMPORTING
...
CLASS zcl_bug_request IMPLEMENTATION.
  ...
  METHOD create_bug.
    " 1. Initialization: Clear returning parameters.
    CLEAR: ev_bug_id, et_messages.

    " 2. Authorization Check: Ensure user has the basic right to create.
    AUTHORITY-CHECK OBJECT 'Z_BUG_AUTH'
      ID 'ACTVT'     FIELD '01'
      ID 'ZBUG_ROLE' FIELD 'BASIC'.
    IF sy-subrc <> 0.
      APPEND VALUE #( type = 'E' id = 'ZBUG' number = '001' message = 'You do not have permission to create a bug.' ) TO et_messages.
      RETURN.
    ENDIF.

    " 3. Validation: Delegate validation to the dedicated validator class.
    ...
    " 4. Generate Bug ID: Get a unique ID from our number range.
    ...
    " 6. Database Transaction: Insert the new bug record.
    INSERT zbug_header FROM ls_bug_header.
    IF sy-subrc <> 0.
      APPEND VALUE #( type = 'A' id = 'ZBUG' number = '003' message = 'Critical DB Error on bug creation. Contact admin.' ) TO et_messages.
      RETURN.
    ENDIF.

    " 7. Log History: Create the first entry in the bug's audit trail.
    me->log_history(
      EXPORTING
        iv_bug_id     = lv_new_bug_id
        iv_action     = 'CREA' " CREA = Created
        iv_new_status = 'N'
        iv_comments   = 'Bug logged successfully.'
    ).

    " 8. Trigger Workflow: Raise an event to start the assignment workflow.
    DATA(lv_objkey) = CONV sweinstcou-objkey( lv_new_bug_id ).
    CALL FUNCTION 'SWE_EVENT_CREATE'
      EXPORTING
        objtype           = 'ZBUS_BUG' " Custom Business Object
        objkey            = lv_objkey
        event             = 'Created'
      EXCEPTIONS
        objtype_not_found = 1
        OTHERS            = 2.
    IF sy-subrc <> 0.
      me->log_message( |Workflow trigger failed for Bug ID { lv_new_bug_id }| ).
    ENDIF.

    " 9. Finalization: Return the new ID and a success message.
  ...
  METHOD update_bug.
    ...
    " 2. Get existing data to compare against.
    ...
    " 3. Authorization Check: A more complex check for updates.
    AUTHORITY-CHECK OBJECT 'Z_BUG_AUTH' ID 'ACTVT' FIELD '02' ID 'ZBUG_ROLE' FIELD 'ADMIN'.
    IF sy-subrc = 0.
      lv_is_admin = abap_true.
    ELSE. " If not admin, check if user is the assigned developer
      AUTHORITY-CHECK OBJECT 'Z_BUG_AUTH' ID 'ACTVT' FIELD '02' ID 'ZBUG_ROLE' FIELD 'WORK'.
    ...
    " 4. Compare incoming values with existing ones and log changes.
    DATA(ls_update_struc) = ls_existing_bug.
    DATA lv_status_changed TYPE abap_bool.

    " Check and log change for ASSIGNED_TO (re-assignment)
    IF is_bug_data-assigned_to IS NOT INITIAL AND is_bug_data-assigned_to <> ls_existing_bug-assigned_to.
      IF lv_is_admin = abap_false. " Business Rule: Only admins can reassign.
          APPEND VALUE #( type = 'E' message = 'Only an Admin can re-assign a bug.' ) TO et_messages.
          RETURN.
      ENDIF.
      me->log_field_change( iv_bug_id = iv_bug_id iv_field_name = 'ASSIGNED_TO' iv_old_value = ls_existing_bug-assigned_to iv_new_value = is_bug_data-assigned_to ).
      ls_update_struc-assigned_to = is_bug_data-assigned_to.
      " Business Rule: When re-assigned, status goes back to 'Assigned'.
      ls_update_struc-status = 'A'.
    ENDIF.

    " Check and log change for STATUS
    IF is_bug_data-status IS NOT INITIAL AND is_bug_data-status <> ls_existing_bug-status.
    ...
    " * TODO: Add more field checks here (e.g., for RESOLUTION, PRIORITY, etc.) *

    " 5. Database Update: Apply the changes to the database.
    ...
    IF lv_status_changed = abap_true.
         me->log_history( iv_bug_id = iv_bug_id iv_action = 'CHGS' iv_new_status = ls_update_struc-status iv_comments = 'Status changed.' ).
    ELSE.
  ...
  METHOD get_bug.
    " Simple retrieval of a single bug record.
    " TODO: Add authorization check to ensure user can view this bug.
    SELECT SINGLE * FROM zbug_header WHERE bug_id = @iv_bug_id INTO @es_bug_data.
    IF sy-subrc <> 0.
  ...
  METHOD generate_bug_id.
    ...
    IF sy-subrc = 0.
      " Format the ID, e.g., BUG-20260101-001
      rv_bug_id = |BUG-{ sy-datum }-{ lv_sequence_nr CWIDTH 3 ALPHA = OUT }|.
    ELSE.
      " Handle error if number range fails.
      CLEAR rv_bug_id.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD log_history.
    ...
    " Get the next sequence number for this bug's history.
    SELECT MAX( sequence_no ) FROM zbug_history
      INTO ls_history-sequence_no
      WHERE bug_id = @iv_bug_id.
    ls_history-sequence_no = ls_history-sequence_no + 1.
  ...
  METHOD log_field_change.
    ...
    " Get the next item number for this bug's item log.
    SELECT MAX( item_no ) FROM zbug_items
        INTO ls_item-item_no
        WHERE bug_id = @iv_bug_id.
    ls_item-item_no = ls_item-item_no + 1.
  ...
  METHOD log_message.
    " TODO: Implement robust application logging, e.g., using SLG1 (Application Log).
